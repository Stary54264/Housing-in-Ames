...
We can see from @fig-fit-resp that the pionts scattered along the diagonal, so conditional mean predictor is fulfilled.
...
We can see some violations of constant variance in @fig-res-1, @fig-res-2, and @fig-res-3 for variables except `year_built`, so a transformation is needed for those variables.
...
R [@r] was used to conduct this research. Packages used include `tidyverse` [@tidyverse], `knitr` [@knitr], `patchwork` [@patchwork], `car` [@car], `leaps` [@leaps], and `MASS` [@MASS]. the dataset used is @ameshousing from @ames.
