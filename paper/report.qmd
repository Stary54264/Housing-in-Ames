...
We choose to edit the introduction by simplicity. Our original introduction contains some clutters. We can improve the introduction clearer and shorter. 

Original text: We will test whether there is a statistically significant linear relationship between certain property characteristics (predictors) and the sale price of houses (response) using linear regression.
Edited text: By setting up the model, we can identify the factors that influence house prices. 

It is easy to delete some complex sentences when read the introduction with the editing techniques. The challenge part is to break and modify sentences in clearer and fewer words. 
